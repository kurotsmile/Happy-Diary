...
public class Sync_data_manager : MonoBehaviour
{
    ...
    private void act_done_show_list_sync(string s_data)
    {
        ...
        if (list_syn.Count <=0)
        {
            ...
            this.app.carrot.show_msg(PlayerPrefs.GetString("syn_title", "Backup and sync calendar data"), PlayerPrefs.GetString("syn_data_no", "You have no backup created yet!"));
            return;
        }

        if (this.box_sync_download != null) this.box_sync_download.close();
        this.box_sync_download = this.app.carrot.Create_Box(PlayerPrefs.GetString("syn_title", "Backup and sync calendar data"), this.icon_sync_download);
    ...
    {
        this.app.carrot.show_msg(PlayerPrefs.GetString("syn_title", "Backup and sync calendar data"), PlayerPrefs.GetString("syn_backup_success","Successful data backup created!"),Msg_Icon.Success);
    }

    public void show_menu_sync()
    {
        this.app.carrot.play_sound_click();
        Carrot_Box box_syn=this.app.carrot.Create_Box(PlayerPrefs.GetString("syn_title", "Backup and sync calendar data"), this.icon_sync);
        Carrot_Box_Item item_syn_download = box_syn.create_item_of_top();
        item_syn_download.set_icon(this.icon_sync_download);
        item_syn_download.set_title(PlayerPrefs.GetString("syn_recover", "Data recovery"));
        item_syn_download.set_tip(PlayerPrefs.GetString("syn_recover_tip", "Recover data from server"));
        ...
        item_syn_upload.set_title(PlayerPrefs.GetString("syn_backup", "Data backup"));
        item_syn_upload.set_tip(PlayerPrefs.GetString("syn_backup_tip", "Store your data on the server"));
